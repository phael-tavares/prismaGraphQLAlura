...
const resolvers = {
 Query: {
   ...
   postsByUser: async (_, args) => prisma.user.findUnique({ where: { id: Number(args.id)}}).posts(),

   postsByReviewer: async (_, args) => prisma.review.findUnique({ where: { id: Number(args.id)}}).reviewer().posts()
 },
